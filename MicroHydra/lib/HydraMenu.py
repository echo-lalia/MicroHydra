...
class Menu:
    def __init__(self, display, font, title: str = None, per_page: int = 3, y_padding: int = 20, ui_color: int = 53243, bg_color: int = 4421, ui_sound: bool = True, volume: int = 2):
        from lib import microhydra as mh
        self.display = displayimport math
...
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ MENU ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
class Menu:
    def __init__(self, display_fbuf, font, sound = None, per_page:int = _PER_PAGE, y_padding:int = _Y_PADDING):
        """
        args:
    ...
    def display_menu(self):
        ...
        self.display.hline(0, y + self.font.HEIGHT, _DISPLAY_WIDTH, config.palette[3])# separation lines
...
class RGB_item:
    ...
    def draw_rgb_win(self):
        win = pop_up_win(self.menu.display, self.text, config['ui_color'], config['bg_color'], self.font)
        win.draw()
        color = [63488, 2016, 31]
        ...
        for i, item in enumerate(self.items):
            ...
            if i == self.cursor_index:
                ...
                self.menu.display.bitmap_text(self.menu.font, str(item), x, y + self.font.HEIGHT, 16777215)#, config['bg_color'])
            self.menu.display.bitmap_text(self.font, str(rgb_text[i]), x, y, color[i])#, config['bg_color'])
...
class do_item:
    ...
    def draw(self):
        if self.selected:
            ...
            self.menu.display.bitmap_text(self.menu.font, TEXT, int(_DISPLAY_WIDTH / 2) - get_text_center(TEXT, self.menu.font), self.y_pos, config['ui_color'])#, config.palette[3])
        else:
            self.menu.display.bitmap_text(self.menu.font, self.text, int(_DISPLAY_WIDTH / 2) - get_text_center(self.text, self.menu.font), self.y_pos, config['ui_color'])#, config['bg_color'])
...
class int_select_item:
    ...
    def draw_win(self):
        win = pop_up_win(self.menu.display, self.text, config['ui_color'], config['bg_color'], self.menu.font)
        win.draw()
        for i in range(0,8):
            self.menu.display.hline(
                x = (119 - i),
        ...
        self.menu.display.bitmap_text(self.menu.font, str(self.current_value), x, 75, config['ui_color'])#, config['bg_color'])
...
class pop_up_win:
    ...
    def draw(self):
        ...
        if self.text and self.font:
            ...
            self.display.bitmap_text(self.font, str(self.text + ":"), center_x, 20, config['ui_color'])
...
def draw_left_text(menu, text:str, font, y_pos:int, selected):
    if selected:
        menu.display.bitmap_text(menu.font, '>', 0, y_pos, config['ui_color'])#, config.palette[3])
        menu.display.bitmap_text(menu.font, text, 15, y_pos, config['ui_color'])#, config.palette[3])
    else:
        menu.display.bitmap_text(menu.font, ' ', 0, y_pos, config['ui_color'])#, config['bg_color'])
        menu.display.bitmap_text(menu.font, text, 15, y_pos, config['ui_color'])#, config['bg_color'])

def draw_right_text(menu, text:str, font, y_pos:int):
    menu.display.fill_rect(160, y_pos, 80, menu.font.HEIGHT, config['bg_color'])# clear word
    ...
    if font.HEIGHT < menu.font.HEIGHT:
        menu.display.bitmap_text(font, str(text), x, int(y_pos + (font.HEIGHT / 2)), color)#, config['bg_color'])
    else:
        menu.display.bitmap_text(font, str(text), x, y_pos, color)#, config['bg_color'])

def draw_text_on_win(menu, text:str, font, y_pos:int):
    if len(text) > int(222 / font.WIDTH):
        menu.display.bitmap_text(menu.font, text[0:int(222 / font.WIDTH)], int(menu.display.width / 2) - get_text_center(text[0:int(222 / font.WIDTH)], menu.font), 85-font.HEIGHT, config['ui_color'])#, config['bg_color'])
        menu.display.bitmap_text(menu.font, text[int(222 / font.WIDTH):len(text)], int(menu.display.width / 2) - get_text_center(text[int(222 / font.WIDTH):len(text)], menu.font), 85, config['ui_color'])#, config['bg_color'])
    else:
        menu.display.bitmap_text(menu.font, text, int(menu.display.width / 2) - get_text_center(text, menu.font), 75, config['ui_color'])#, config['bg_color'])
        self.items = []
        self.cursor_index = 0
        self.prev_cursor_index = 0
        self.setting_screen_index = 0
