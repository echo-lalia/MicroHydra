"""Driver for the TCA8418 I2C Keypad/GPIO expander."""
...
class TCA8418:
    ...
    def __init__(self, i2c: I2C, address: int = TCA8418_ADDR):
        """Initialize the device with the given I2C object, and address."""
        self.i2c = i2c
    ...
    def _write_register(self, register: int, value: int):
        """Write a byte value to a specific register using writeto_mem."""
        self.i2c.writeto_mem(self.address, register, bytes([value]))

    def _read_register(self, register: int) -> int:
        """Read a byte value from a specific register using readfrom_mem."""
        # Return the single byte value
        return self.i2c.readfrom_mem(self.address, register, 1)[0]

    def init_device(self):
        """Initialize the TCA8418 for keypad scanning mode and clear interrupts."""
    ...
    def read_event_count(self) -> int:
        """Get the number of events in the FIFO - including press and release."""
        return self._read_register(REG_KEY_LCK_EC) & 0x0F

    def read_key_event(self) -> tuple[int, bool]:
        """Read the next key event from the FIFO queue."""
    ...
    def clear_interrupt(self):
        """Clear the interrupt status by reading/writing to the INT_STAT register."""
